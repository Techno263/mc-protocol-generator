...
class CompoundVar:
    ...
    def __len__(self):
        return dl.string_size(self.string) + dl.varlong(self.varlong) + dl.short_size
...
class ComplexPacket:
    ...
    def read_packet(reader):
        array_comp = [ArrayCompItem.read_data(reader) for _ in range(reader.read_varint())]
        array_int = [reader.read_int() for _ in range(reader.read_varint())]
        array_varint = [reader.read_varint() for _ in range(reader.read_varint())]
