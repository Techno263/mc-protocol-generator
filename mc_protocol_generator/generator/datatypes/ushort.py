...
class UShort(Base):
    def len_str(self, sizer_name):
        return '%s.ushort_size{{len_body}}' % (sizer_name)
    ...
    def write_str(self, writer_name):
        field_name = format_field_name(self.name)
        return '%s.write_ushort(self.%s); {{write_packet_body}}' % (writer_name, field_name)

    def read_str(self, reader_name):
        field_name = format_field_name(self.name)
        return '%s = %s.read_ushort(); {{read_packet_body}}' % (field_name, reader_name)
