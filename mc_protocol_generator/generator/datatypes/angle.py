...
class Angle(Base):
    ...
    def len_str(self, sizer_name):
        return '%s.angle_size{{len_body}}' % (sizer_name)

    def repr_str(self):
        field_name = format_field_name(self.name)
        return '%s={repr(self.%s)}{{repr_body}}' % (field_name, field_name)

    def write_str(self, writer_name):
        field_name = format_field_name(self.name)
        return '%s.write_angle(self.%s);{{write_packet_body}}' % (writer_name, field_name)

    def read_str(self, reader_name):
        field_name = format_field_name(self.name)
        return '%s = %s.read_angle();{{read_packet_body}}' % (field_name, reader_name)
